export const arStorefront: Record<string, string> = {
  // ── Global / metadata ──────────────────────────────────────────────
  "Personalized Books for Kids | Custom Storybooks - mon petit hero":
    "كتب مخصصة للأطفال | قصص مخصصة - mon petit hero",
  "Create unique kids' storybooks with Mon Petit Hero. Upload photos and watch them become part of personalized stories your child will treasure forever.":
    "أنشئ كتب قصص فريدة لأطفالك مع Mon Petit Hero. ارفع الصور وشاهدها تتحول إلى حكايات مخصصة سيحتفظ بها طفلك إلى الأبد.",
  Wonderwraps: "Wonderwraps",
  "Sign in to Mon Petit Hero to manage your storybooks.":
    "سجّل الدخول إلى Mon Petit Hero لإدارة كتب القصص الخاصة بك.",
  "Create your free Mon Petit Hero account.":
    "أنشئ حسابك المجاني في Mon Petit Hero.",

  // ── Navigation / header ────────────────────────────────────────────
  Home: "الرئيسية",
  Books: "الكتب",
  "My Books": "كتبي",
  Support: "الدعم",
  Dashboard: "لوحة التحكم",
  Library: "المكتبة",
  "Create Story": "أنشئ قصة",
  Admin: "المشرف",
  "Shop Books": "تسوق الكتب",
  "Shop books": "تسوق الكتب",
  "Sign in": "تسجيل الدخول",
  "Sign In": "تسجيل الدخول",
  "Start Creating": "ابدأ الإنشاء",
  "Open menu": "افتح القائمة",
  "Save 20% on 2+ books using code": "وفّر 20% عند شراء كتابين أو أكثر بالرمز",
  "Go to My Books": "الانتقال إلى كتبي",
  "Personalise now": "خصّص الآن",
  "View all": "عرض الكل",
  "View all books": "عرض جميع الكتب",
  "Try for free": "جرّبه مجانًا",
  Discover: "استكشف",
  "Browse all": "تصفح الكل",
  "Close": "إغلاق",

  // ── Home page ──────────────────────────────────────────────────────
  Bestsellers: "الأكثر مبيعًا",
  "Personalise a bestseller": "خصّص كتابًا من الأكثر مبيعًا",
  "New releases": "إصدارات جديدة",
  "Discover what's new": "اكتشف الجديد",
  "Our books": "كتبنا",
  "Books for your little girl!": "كتب لفتاتك الصغيرة!",
  "Books for your little boy!": "كتب لصغيرك!",
  "Create unique storybook": "أنشئ كتاب قصة فريدًا",
  "Craft magical tales where you're the hero":
    "اصنع حكايات سحرية يكون فيها طفلك هو البطل",
  "Upload a photo, pick a story, and your child becomes the main character — printed, bound, and delivered to your door.":
    "ارفع صورة، اختر قصة، وسيصبح طفلك الشخصية الرئيسية — مطبوعة ومجلدة وتصل حتى باب منزلك.",
  "Create your book in minutes": "أنشئ كتابك في دقائق",
  "How Mon Petit Hero works": "كيف يعمل Mon Petit Hero",
  "Pick a storybook": "اختر كتاب قصة",
  "Add your child's picture": "أضف صورة طفلك",
  "Preview and order": "معاينة وطلب",
  "Your story is printed with care and delivered with joy.":
    "تُطبع قصتك بعناية ويتم توصيلها بفرح.",
  "Browse stories by age": "تصفح القصص حسب العمر",
  "Age 2-4": "عمر 2-4",
  "Age 4-6": "عمر 4-6",
  "Age 6-8": "عمر 6-8",
  "Bring your child's imagination to life!":
    "أحضر خيال طفلك إلى الحياة!",
  "Make them the hero of their own magical adventure with a hyper-personalised storybook.":
    "اجعل طفلك بطل مغامرته السحرية الخاصة مع كتاب قصة مخصص للغاية.",
  "Customize faces, expressions, and angles":
    "خصّص الوجوه والتعابير والزوايا",
  "To bring your character to life!": "لإضفاء الحياة على شخصيتك!",
  "Many styles": "أنماط متعددة",
  "Full of expressions": "مليء بالتعابير",
  "Different angles": "زوايا مختلفة",
  "Style one": "النمط ١",
  "Style two": "النمط ٢",
  "Style three": "النمط ٣",
  "Expression one": "التعبير ١",
  "Expression two": "التعبير ٢",
  "Expression three": "التعبير ٣",
  "Angle one": "الزاوية ١",
  "Angle two": "الزاوية ٢",
  "Angle three": "الزاوية ٣",
  "Personalised stories that celebrate their big dreams":
    "قصص مخصصة تحتفل بأحلامهم الكبيرة",
  "Inspire their dreams with hyper-personalised career adventures!":
    "ألهم أحلامهم بمغامرات مهنية مخصصة للغاية!",
  "Explore careers": "استكشف المهن",
  Firefighter: "رجل إطفاء",
  "Police Officer": "شرطي",
  Pilot: "طيّار",
  Doctor: "طبيب",
  "Frequently asked questions": "الأسئلة الشائعة",
  "See all questions": "عرض جميع الأسئلة",

  // ── FAQs (home + page) ─────────────────────────────────────────────
  "How do I place an order?": "كيف أضع طلبًا؟",
  "It's easy! Choose the book you want personalised, upload a photo of your child (make sure it matches our recommendations), and enter their name and age. You'll then get a preview of the book. If you're happy with it, just proceed to payment to complete your order.":
    "الأمر سهل! اختر الكتاب الذي تريد تخصيصه، ارفع صورة لطفلك (تأكد من مطابقتها لتوصياتنا)، ثم أدخل اسمه وعمره. ستحصل بعدها على معاينة للكتاب. وإن أعجبك، انتقل إلى الدفع لإتمام طلبك.",
  "Do you ship to my location?": "هل تشحنون إلى موقعي؟",
  "Yes! We ship to over 200 countries and regions, so wherever you are, we'll make sure your order reaches you. Simply enter your shipping details at checkout, and we'll take care of the rest.":
    "نعم! نشحن إلى أكثر من 200 دولة ومنطقة، فأينما كنت سنضمن وصول طلبك إليك. فقط أدخل بيانات الشحن عند إتمام الطلب وسنتكفل بالباقي.",
  "Can I get a refund for my order?": "هل يمكنني استرداد أموالي؟",
  "Yes, you can receive a full refund if your book hasn't been printed yet, or a partial refund if it has been printed but not yet shipped. Once the book has been printed and shipped, we're unable to offer a refund. To request a refund contact us through our support page or by email at support@monpetithero.shop.":
    "نعم، يمكنك الحصول على استرداد كامل إذا لم يُطبع كتابك بعد، أو استرداد جزئي إذا طُبع ولم يُشحن بعد. وبمجرد طباعة الكتاب وشحنه، لا يمكننا تقديم استرداد. لطلب استرداد، تواصل معنا عبر صفحة الدعم أو عبر البريد الإلكتروني support@monpetithero.shop.",
  "How long does shipping take?": "كم تستغرق مدة الشحن؟",
  "Shipping times depend on the shipping method you choose at checkout. Standard shipping usually takes 10 to 30 business days, while express shipping typically arrives within 7 to 20 business days. Delivery times include only business days, so holidays or weekends may cause slight delays.":
    "تعتمد مدة الشحن على طريقة الشحن التي تختارها عند إتمام الطلب. يستغرق الشحن العادي عادة من 10 إلى 30 يوم عمل، بينما يصل الشحن السريع عادة خلال 7 إلى 20 يوم عمل. تشمل أوقات التسليم أيام العمل فقط، لذا قد تتسبب العطلات أو عطلات نهاية الأسبوع في تأخيرات بسيطة.",
  "Will I have to pay duties or extra fees?":
    "هل سأدفع رسومًا جمركية أو رسومًا إضافية؟",
  "The prices listed on our website do not include any additional taxes, customs duties, or import fees. These charges may apply depending on your country's regulations and are the responsibility of the recipient. We recommend checking with your local customs office for more information.":
    "الأسعار المعروضة على موقعنا لا تشمل الضرائب الإضافية أو الرسوم الجمركية أو رسوم الاستيراد. قد تُطبَّق هذه الرسوم حسب لوائح بلدك وتكون مسؤولية المستلم. ننصح بالاستعلام من مكتب الجمارك المحلي الخاص بك.",
  "What if I'm not happy with my order?":
    "ماذا لو لم أكن راضيًا عن طلبي؟",
  "After payment, you'll review and approve your book. If you're not happy with it, you can request changes, and our dedicated support team will be happy to assist you.":
    "بعد الدفع، ستُراجع كتابك وتوافق عليه. وإذا لم يعجبك، يمكنك طلب تعديلات وسيسعد فريق الدعم المخصص لدينا بمساعدتك.",
  "How can I reach customer support?": "كيف أتواصل مع دعم العملاء؟",
  "You can contact our customer support team anytime through our support page or by emailing support@monpetithero.shop.":
    "يمكنك التواصل مع فريق دعم العملاء في أي وقت عبر صفحة الدعم أو عبر البريد الإلكتروني support@monpetithero.shop.",
  "What languages are your books available in?":
    "بأي لغات تتوفر كتبكم؟",
  "Our books are currently available in English, Spanish, Portuguese (Brazil), Arabic, French, Turkish, German, Italian, Dutch and Albanian. We are actively working on adding more languages soon.":
    "كتبنا متوفرة حاليًا بالإنجليزية والإسبانية والبرتغالية (البرازيل) والعربية والفرنسية والتركية والألمانية والإيطالية والهولندية والألبانية. ونعمل بنشاط على إضافة المزيد من اللغات قريبًا.",
  "Need help?": "هل تحتاج إلى مساعدة؟",
  "Everything you need to know about personalising, ordering and receiving your Mon Petit Hero books.":
    "كل ما تحتاج معرفته حول تخصيص وطلب واستلام كتب Mon Petit Hero.",
  "Still have a question?": "هل ما زال لديك سؤال؟",
  "Our support team is happy to help — reach out any time.":
    "فريق الدعم لدينا يسعد بمساعدتك — تواصل معنا في أي وقت.",

  // ── Footer ─────────────────────────────────────────────────────────
  "About Mon Petit Hero": "عن Mon Petit Hero",
  "Contact us": "اتصل بنا",
  FAQ: "الأسئلة الشائعة",
  FAQs: "الأسئلة الشائعة",
  Blog: "المدونة",
  "Customer Area": "منطقة العميل",
  "My Account": "حسابي",
  Orders: "طلباتي",
  Terms: "الشروط",
  "Privacy Policy": "سياسة الخصوصية",
  "Create hyper-personalised storybooks that make your child the hero, with quick customisation and speedy delivery.":
    "أنشئ كتب قصص مخصصة للغاية تجعل طفلك بطل القصة، مع تخصيص سريع وتوصيل سريع.",
  "Facebook": "فيسبوك",
  "Instagram": "إنستغرام",
  "TikTok": "تيك توك",
  "Subscribe to our newsletter": "اشترك في نشرتنا الإخبارية",
  "Don't miss out on the newest books.": "لا تفوّت أحدث الكتب.",
  "Email address": "البريد الإلكتروني",
  Subscribe: "اشترك",
  "Subscribed. Check your inbox for a confirmation.":
    "تم الاشتراك. تحقق من بريدك الوارد للحصول على التأكيد.",
  "All rights reserved": "جميع الحقوق محفوظة",

  // ── Cards / filler ─────────────────────────────────────────────────
  "Video placeholder": "فيديو",
  "Your browser does not support the video tag.":
    "متصفحك لا يدعم وسوم الفيديو.",

  // ── Books listing ──────────────────────────────────────────────────
  "Personalised Books": "كتب مخصصة",
  "Browse personalised storybooks that make your child the hero. Choose from adventures, princess tales, space journeys and more.":
    "تصفح كتب قصص مخصصة تجعل طفلك البطل. اختر من بين المغامرات وحكايات الأميرات ورحلات الفضاء والمزيد.",
  "Mon Petit Hero collection": "مجموعة Mon Petit Hero",
  "Personalised storybooks": "كتب قصص مخصصة",
  "Your child is the hero of every page. Pick a story, add their photo and name, and receive a beautifully printed book they'll treasure forever.":
    "طفلك هو بطل كل صفحة. اختر قصة، وأضف صورته واسمه، واحصل على كتاب مطبوع جميل سيحتفظ به طفلك إلى الأبد.",
  All: "الكل",
  "Showing books for": "عرض الكتب لعمر",
  "Can't decide? Start with our bestsellers":
    "لا تستطيع الاختيار؟ ابدأ بأكثر كتبنا مبيعًا",
  "Every book is fully personalised with your child's photo, name and age — and each printed book order unlocks an extra free story.":
    "كل كتاب مخصص بالكامل بالصورة والاسم وعمر طفلك — وكل طلب لكتاب مطبوع يفتح قصة إضافية مجانية.",
  "Create my child's story": "أنشئ قصة طفلي",

  // ── Book detail ────────────────────────────────────────────────────
  "Book not found": "الكتاب غير موجود",
  "Personalise this storybook so your child becomes the hero.":
    "خصّص هذا الكتاب ليكون طفلك هو البطل.",
  "Back to all books": "العودة إلى جميع الكتب",
  "Personalised book": "كتاب مخصص",
  "This is the story where": "هذه هي القصة التي",
  "becomes the adventure your child is at the center of. Upload a photo of your child, add their first name and age, and watch as they become the hero of this beautifully illustrated, printed storybook.":
    "يعيش فيها طفلك مغامرته الخاصة. ارفع صورة لطفلك وأضف اسمه وعمره، وشاهد كيف يصبح بطل هذا الكتاب المصوّر والمطبوع الجميل.",
  "Personalise this book": "خصّص هذا الكتاب",
  "Talk to support": "تحدث إلى الدعم",
  "Your child is the hero": "طفلك هو البطل",
  "Their photo, name and age are woven into every scene.":
    "صورته واسمه وعمره تُدمج في كل مشهد.",
  "Personalised to perfection": "تخصيص مثالي",
  "Pick names, characters and details to make it truly theirs.":
    "اختر الأسماء والشخصيات والتفاصيل ليكون فريدًا حقًا.",
  "Printed with care, shipped fast": "مطبوع بعناية ويُشحن بسرعة",
  "Premium softcover books delivered right to your door.":
    "كتب فاخرة بغلاف مرن تُوصَل مباشرة إلى باب منزلك.",
  "More stories to love": "المزيد من القصص الرائعة",

  // ── Support ────────────────────────────────────────────────────────
  "We're here to help": "نحن هنا لمساعدتك",
  "How can we help you?": "كيف يمكننا مساعدتك؟",
  "Find answers to common questions, get in touch with the team, or catch up on the details that matter.":
    "اعثر على إجابات للأسئلة الشائعة، تواصل مع الفريق، أو اطّلع على التفاصيل المهمة.",
  "Quick answers on ordering, personalisation, shipping, refunds and more.":
    "إجابات سريعة عن الطلب والتخصيص والشحن والاسترداد والمزيد.",
  "Read FAQs": "اقرأ الأسئلة الشائعة",
  "Send the team a message and hear back within 24 hours.":
    "أرسل رسالة إلى الفريق وستتلقى ردًا خلال 24 ساعة.",
  "Contact the team": "تواصل مع الفريق",
  "Terms & Conditions": "الشروط والأحكام",
  "The fine print on orders, prints, payments and our promise to you.":
    "التفاصيل الدقيقة حول الطلبات والطباعة والمدفوعات ووعدنا لك.",
  "Read terms": "اقرأ الشروط",
  "How we handle the photos, names and details you share with us.":
    "كيف نتعامل مع الصور والأسماء والبيانات التي تشاركها معنا.",
  "Read privacy policy": "اقرأ سياسة الخصوصية",
  "Need a human? Email us at": "تريد التحدث مع إنسان؟ راسلنا على",
  "Send a message": "أرسل رسالة",

  // ── Terms & Conditions ─────────────────────────────────────────────
  "Back to support": "العودة إلى الدعم",
  "Last updated: January 2026": "آخر تحديث: يناير 2026",
  "1. The service": "1. الخدمة",
  "monpetithero.shop sells personalised children's storybooks. Every book is created individually from details you provide, including your child's photo, name and age.":
    "يبيع monpetithero.shop كتب قصص مخصصة للأطفال. يُنشأ كل كتاب بشكل فردي من التفاصيل التي تقدمها، بما في ذلك صورة طفلك واسمه وعمره.",
  "2. Orders & payment": "2. الطلبات والدفع",
  "All prices are shown in the currency you select. Payment is taken at checkout and your book is printed after you approve the preview. Each printed book order unlocks one extra free story generation on your account.":
    "تُعرض جميع الأسعار بالعملة التي تختارها. يُؤخذ الدفع عند إتمام الطلب ويُطبع كتابك بعد موافقتك على المعاينة. كل طلب كتاب مطبوع يفتح توليد قصة إضافية مجانية على حسابك.",
  "3. Refunds": "3. الاسترداد",
  "You can receive a full refund if your book has not been printed yet, or a partial refund if it has been printed but not yet shipped. Once printed and shipped, we are unable to offer a refund. Contact us at support@monpetithero.shop to request one.":
    "يمكنك الحصول على استرداد كامل إذا لم يُطبع كتابك بعد، أو استرداد جزئي إذا طُبع ولم يُشحن بعد. وبمجرد الطباعة والشحن، لا يمكننا تقديم استرداد. تواصل معنا على support@monpetithero.shop لطلب استرداد.",
  "4. Shipping & duties": "4. الشحن والرسوم",
  "Standard shipping usually takes 10–30 business days; express takes 7–20 business days. Prices listed do not include taxes, customs duties or import fees, which are the recipient's responsibility.":
    "يستغرق الشحن العادي عادة من 10 إلى 30 يوم عمل، والشحن السريع من 7 إلى 20 يوم عمل. الأسعار المعروضة لا تشمل الضرائب أو الرسوم الجمركية أو رسوم الاستيراد، وهي مسؤولية المستلم.",
  "5. Your content": "5. المحتوى الخاص بك",
  "You retain all rights to the photos and details you provide. We only use them to create and print your order and never share them with third parties.":
    "تحتفظ بجميع الحقوق في الصور والبيانات التي تقدمها. نستخدمها فقط لإنشاء وطباعة طلبك ولا نشاركها أبدًا مع أطراف ثالثة.",
  "6. Contact": "6. التواصل",
  "Questions about these terms? Reach us at support@monpetithero.shop.":
    "لديك أسئلة حول هذه الشروط؟ تواصل معنا على support@monpetithero.shop.",
  "The terms and conditions that apply to ordering personalised storybooks from Mon Petit Hero.":
    "الشروط والأحكام المطبقة على طلب كتب القصص المخصصة من Mon Petit Hero.",

  // ── Privacy Policy ─────────────────────────────────────────────────
  "1. What we collect": "1. ما الذي نجمعه",
  "To create your personalised books we collect your contact details (name, email, shipping address) and the personalised content you provide, such as your child's photo, name and age.":
    "لإنشاء كتبك المخصصة، نجمع بيانات الاتصال الخاصة بك (الاسم والبريد الإلكتروني وعنوان الشحن) والمحتوى المخصص الذي تقدمه، مثل صورة طفلك واسمه وعمره.",
  "2. How we use it": "2. كيف نستخدمها",
  "We use this information to build and print your order, keep you updated on its progress, and improve our service. We never sell your personal data to anyone.":
    "نستخدم هذه المعلومات لإنشاء وطباعة طلبك، وإطلاعك على تقدمه، وتحسين خدمتنا. لا نبيع بياناتك الشخصية لأي شخص أبدًا.",
  "3. Photos & children's information": "3. الصور ومعلومات الأطفال",
  "The photos and details you upload are used only to generate your book. They are processed securely and are never published, shared or used for any other purpose without your consent.":
    "تُستخدم الصور والبيانات التي ترفعها فقط لتوليد كتابك. تتم معالجتها بأمان ولا تُنشر أو تُشارك أو تُستخدم لأي غرض آخر دون موافقتك.",
  "4. Payments": "4. المدفوعات",
  "Card payments are processed by trusted third-party payment providers. We do not store your full card details on our servers.":
    "تتم معالجة مدفوعات البطاقات بواسطة مزودي دفع موثوقين من جهات خارجية. لا نخزن بيانات بطاقتك كاملة على خوادمنا.",
  "5. Your rights": "5. حقوقك",
  "You may request a copy, correction or deletion of your personal data at any time by emailing support@monpetithero.shop.":
    "يمكنك طلب نسخة أو تصحيح أو حذف بياناتك الشخصية في أي وقت عبر البريد الإلكتروني support@monpetithero.shop.",
  "How Mon Petit Hero collects, uses and protects the personal details and photos you share with us.":
    "كيف يجمع Mon Petit Hero ويستخدم ويحمي المعلومات الشخصية والصور التي تشاركها معنا.",

  // ── Contact ────────────────────────────────────────────────────────
  "We'd love to hear from you": "يسعدنا التواصل معك",
  "Questions about an order, a personalisation, or the perfect book for your child? Send us a message.":
    "لديك سؤال حول طلب أو تخصيص أو الكتاب المثالي لطفلك؟ أرسل لنا رسالة.",
  "Email us": "راسلنا عبر البريد",
  "Chat with us": "تحدث معنا",
  "On Facebook, Instagram and TikTok": "على فيسبوك وإنستغرام وتيك توك",
  "Response time": "مدة الرد",
  "We usually reply within 24 hours.": "نرد عادةً خلال 24 ساعة.",
  "Message sent!": "تم إرسال الرسالة!",
  "Thanks for reaching out. Our team will get back to you within 24 hours.":
    "شكرًا لتواصلك معنا. سيعود فريقنا إليك خلال 24 ساعة.",
  Name: "الاسم",
  "Your name": "اسمك",
  Email: "البريد الإلكتروني",
  Subject: "الموضوع",
  "What is this about?": "بماذا يتعلق الموضوع؟",
  Message: "الرسالة",
  "Tell us how we can help…": "أخبرنا كيف يمكننا مساعدتك...",
  "Send message": "إرسال الرسالة",

  // ── Blog ───────────────────────────────────────────────────────────
  "The Mon Petit Hero blog": "مدونة Mon Petit Hero",
  "Stories behind the stories": "القصص وراء القصص",
  "Ideas, tips and behind-the-scenes peeks to help you raise little readers and big dreamers.":
    "أفكار ونصائح ولمحات من خلف الكواليس لتربية قرّاء صغار وحالمين كبار.",
  "Bedtime tips": "نصائح وقت النوم",
  "Why personalised books make bedtime easier":
    "لماذا تسهّل الكتب المخصصة وقت النوم",
  "When your child is the hero, they beg for one more page. Here's the science behind personalised bedtime stories — and how to make them a habit.":
    "عندما يكون طفلك هو البطل، يطلب صفحة إضافية دائمًا. إليك العلم وراء قصص النوم المخصصة — وكيف تجعلها عادة.",
  "Coming soon": "قريبًا",
  Ideas: "أفكار",
  "10 adventures to spark your child's imagination":
    "10 مغامرات لإشعال خيال طفلك",
  "From dragon rescues to cosmic journeys, these story ideas are a great starting point for your child's very own book.":
    "من إنقاذ التنانين إلى الرحلات الكونية، هذه الأفكار القصصية نقطة بداية رائعة لكتاب طفلك الخاص.",
  News: "الأخبار",
  "Personalisation: how your child's book is made":
    "التخصيص: كيف يُصنع كتاب طفلك",
  "A peek behind the scenes at how we turn a photo, a name and a story idea into a beautifully printed keepsake.":
    "نظرة خلف الكواليس على كيف نحول صورة واسمًا وفكرة قصة إلى تذكار مطبوع جميل.",
  "Read more": "اقرأ المزيد",
  "Meanwhile, why not start your child's own story?":
    "في هذه الأثناء، لماذا لا تبدأ قصة طفلك الخاصة؟",
  "Create a personalised book": "أنشئ كتابًا مخصصًا",

  // ── Not found / errors ─────────────────────────────────────────────
  "This page turned into a blank book": "تحولت هذه الصفحة إلى كتاب فارغ",
  "We couldn't find what you were looking for.":
    "لم نتمكن من العثور على ما تبحث عنه.",
  "Back home": "العودة إلى الرئيسية",
  "Something went wrong": "حدث خطأ ما",
  "We hit a snag while loading this page. You can retry, or head back home.":
    "حدثت مشكلة أثناء تحميل هذه الصفحة. يمكنك إعادة المحاولة أو العودة إلى الرئيسية.",
  "Try again": "أعد المحاولة",
  "Go home": "اذهب إلى الرئيسية",
  "An unexpected error occurred.": "حدث خطأ غير متوقع.",
  Reload: "إعادة التحميل",

  // ── Auth pages ─────────────────────────────────────────────────────
  "Welcome back": "مرحبًا بعودتك",
  "Sign in to continue your stories.": "سجّل الدخول لمتابعة قصصك.",
  "Create your account": "أنشئ حسابك",
  "Every new account starts with free story generations.":
    "يبدأ كل حساب جديد بتوليدات قصص مجانية.",
  "Redirecting to My Books...": "جارٍ إعادة التوجيه إلى كتبي...",

  // ── Welcome ────────────────────────────────────────────────────────
  Welcome: "مرحبًا",
  "Let's create your first story hero": "لننشئ بطل قصتك الأولى",
  "Follow these quick steps. You'll be reading magical bedtime stories in no time.":
    "اتبع هذه الخطوات السريعة وستقرأ قصصًا سحرية قبل النوم في وقت قصير.",
  "Add your child's photo": "أضف صورة طفلك",
  "One clear, smiling photo keeps the hero recognizable on every page.":
    "صورة واضحة ومبتسمة تجعل البطل معروفًا في كل صفحة.",
  "Generate the story": "ولّد القصة",
  "Pick a theme and we write and illustrate it in minutes.":
    "اختر ثيمة وسنكتبها ونرسمها في دقائق.",
  "Read it or order a printed book": "اقرأها أو اطلب كتابًا مطبوعًا",
  "Every printed book you order unlocks 1 extra free story.":
    "كل كتاب مطبوع تطلبه يفتح قصة مجانية إضافية واحدة.",
  "Create My First Story": "أنشئ قصتي الأولى",
  "Browse Templates": "تصفح القوالب",

  // ── Upload / camera / pack / shared ────────────────────────────────
  "Uploading photos...": "جارٍ رفع الصور...",
  "Processing...": "جارٍ المعالجة...",
  "Drag & drop photos here": "اسحب الصور وأفلتها هنا",
  "or click to browse": "أو انقر للاستعراض",
  "Select Photos": "اختر الصور",
  "Supports JPG, PNG, WEBP • Max 50 files":
    "يدعم JPG وPNG وWEBP • حتى 50 ملفًا",
  "Your Gallery": "معرضك",
  "No images yet. Start by generating some!":
    "لا توجد صور بعد. ابدأ بتوليد بعضها!",
  "Image Preview": "معاينة الصورة",
  "Download Image": "تنزيل الصورة",
  "No image yet": "لا توجد صورة بعد",
  "Generated image": "صورة مولدة",
  "Loading image": "جارٍ تحميل الصورة",
  "Image coming soon": "صورة قادمة قريبًا",
  "Loading…": "جارٍ التحميل...",
  "Opening your book…": "جارٍ فتح كتابك...",
  "Previous slide": "الشرائح السابقة",
  "Next slide": "الشريحة التالية",
  "Starting pack generation...": "جارٍ بدء توليد المجموعة...",
  "Pack generation started successfully!": "بدأ توليد المجموعة بنجاح!",
  "Failed to start generation": "فشل بدء التوليد",

  // ── Dynamic fragments (From $) ─────────────────────────────────────
  "From ": "من ",
};