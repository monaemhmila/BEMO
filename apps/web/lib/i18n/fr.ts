export const frStorefront: Record<string, string> = {
  // ── Global / metadata ──────────────────────────────────────────────
  "Personalized Books for Kids | Custom Storybooks - mon petit hero":
    "Livres personnalisés pour enfants | Histoires sur mesure - mon petit hero",
  "Create unique kids' storybooks with Mon Petit Hero. Upload photos and watch them become part of personalized stories your child will treasure forever.":
    "Créez des livres d'histoires uniques avec Mon Petit Hero. Téléchargez des photos et regardez-les devenir des histoires personnalisées que votre enfant chérira pour toujours.",
  Wonderwraps: "Wonderwraps",
  "Sign in to Mon Petit Hero to manage your storybooks.":
    "Connectez-vous à Mon Petit Hero pour gérer vos livres d'histoires.",
  "Create your free Mon Petit Hero account.":
    "Créez votre compte Mon Petit Hero gratuitement.",

  // ── Navigation / header ────────────────────────────────────────────
  Home: "Accueil",
  Books: "Livres",
  Stickers: "Autocollants",
  "My Books": "Mes livres",
  Support: "Assistance",
  Dashboard: "Tableau de bord",
  Library: "Bibliothèque",
  "Create Story": "Créer une histoire",
  Admin: "Admin",
  "Shop Books": "Boutique de livres",
  "Shop books": "Boutique de livres",
  "Sign in": "Se connecter",
  "Sign In": "Se connecter",
  "Start Creating": "Commencer à créer",
  "Open menu": "Ouvrir le menu",
  "Save 20% on 2+ books using code": "Économisez 20% sur 2+ livres avec le code",
  "Go to Dashboard": "Aller au tableau de bord",
  "Personalise now": "Personnaliser maintenant",
  "View all": "Voir tout",
  "View all books": "Voir tous les livres",
  "Try for free": "Essayez gratuitement",
  Discover: "Découvrir",
  "Browse all": "Tout parcourir",
  "Close": "Fermer",

  // ── Home page ──────────────────────────────────────────────────────
  Bestsellers: "Best-sellers",
  "Personalise a bestseller": "Personnalisez un best-seller",
  "New releases": "Nouveautés",
  "Discover what's new": "Découvrez les nouveautés",
  "Our books": "Nos livres",
  "Books for your little girl!": "Des livres pour votre petite fille !",
  "Books for your little boy!": "Des livres pour votre petit garçon !",
  "Create unique storybook": "Créez un livre d'histoires unique",
  "Craft magical tales where you're the hero":
    "Créez des contes magiques où votre enfant est le héros",
  "Upload a photo, pick a story, and your child becomes the main character — printed, bound, and delivered to your door.":
    "Téléchargez une photo, choisissez une histoire, et votre enfant devient le personnage principal — imprimé, relié et livré chez vous.",
  "Create your book in minutes": "Créez votre livre en quelques minutes",
  "How Mon Petit Hero works": "Comment fonctionne Mon Petit Hero",
  "Pick a storybook": "Choisissez une histoire",
  "Add your child's picture": "Ajoutez la photo de votre enfant",
  "Preview and order": "Aperçu et commande",
  "Your story is printed with care and delivered with joy.":
    "Votre histoire est imprimée avec soin et livrée avec joie.",
  "Browse stories by age": "Parcourez les histoires par âge",
  "Age 2-4": "2-4 ans",
  "Age 4-6": "4-6 ans",
  "Age 6-8": "6-8 ans",
  "Bring your child's imagination to life!":
    "Donnez vie à l'imagination de votre enfant !",
  "Make them the hero of their own magical adventure with a hyper-personalised storybook.":
    "Faites-en le héros de sa propre aventure magique avec un livre d'histoires hyper-personnalisé.",
  "Customize faces, expressions, and angles":
    "Personnalisez les visages, les expressions et les angles",
  "To bring your character to life!": "Pour donner vie à votre personnage !",
  "Many styles": "De nombreux styles",
  "Full of expressions": "Riche en expressions",
  "Different angles": "Différents angles",
  "Style one": "Style 1",
  "Style two": "Style 2",
  "Style three": "Style 3",
  "Expression one": "Expression 1",
  "Expression two": "Expression 2",
  "Expression three": "Expression 3",
  "Angle one": "Angle 1",
  "Angle two": "Angle 2",
  "Angle three": "Angle 3",
  "Personalised stories that celebrate their big dreams":
    "Des histoires personnalisées qui célèbrent leurs grands rêves",
  "Inspire their dreams with hyper-personalised career adventures!":
    "Inspirez leurs rêves avec des aventures métiers hyper-personnalisées !",
  "Explore careers": "Explorer les métiers",
  Firefighter: "Pompier",
  "Police Officer": "Policier",
  Pilot: "Pilote",
  Doctor: "Médecin",
  "Frequently asked questions": "Questions fréquentes",
  "See all questions": "Voir toutes les questions",

  // ── FAQs (home + page) ─────────────────────────────────────────────
  "How do I place an order?": "Comment passer une commande ?",
  "It's easy! Choose the book you want personalised, upload a photo of your child (make sure it matches our recommendations), and enter their name and age. You'll then get a preview of the book. If you're happy with it, just proceed to payment to complete your order.":
    "C'est simple ! Choisissez le livre à personnaliser, téléchargez une photo de votre enfant (assurez-vous qu'elle respecte nos recommandations), puis saisissez son nom et son âge. Vous recevrez ensuite un aperçu du livre. S'il vous convient, procédez au paiement pour finaliser votre commande.",
  "Do you ship to my location?":
    "Livrez-vous dans mon pays ?",
  "Yes! We ship to over 200 countries and regions, so wherever you are, we'll make sure your order reaches you. Simply enter your shipping details at checkout, and we'll take care of the rest.":
    "Oui ! Nous livrons dans plus de 200 pays et régions. Où que vous soyez, nous veillerons à ce que votre commande vous parvienne. Saisissez simplement vos coordonnées de livraison au moment du paiement, et nous nous occupons du reste.",
  "Can I get a refund for my order?":
    "Puis-je obtenir un remboursement pour ma commande ?",
  "Yes, you can receive a full refund if your book hasn't been printed yet, or a partial refund if it has been printed but not yet shipped. Once the book has been printed and shipped, we're unable to offer a refund. To request a refund contact us through our support page or by email at support@monpetithero.shop.":
    "Oui, vous pouvez obtenir un remboursement intégral si votre livre n'a pas encore été imprimé, ou un remboursement partiel s'il a été imprimé mais pas encore expédié. Une fois le livre imprimé et expédié, nous ne pouvons pas proposer de remboursement. Pour en demander un, contactez-nous via notre page d'assistance ou par e-mail à support@monpetithero.shop.",
  "How long does shipping take?":
    "Quels sont les délais de livraison ?",
  "Shipping times depend on the shipping method you choose at checkout. Standard shipping usually takes 10 to 30 business days, while express shipping typically arrives within 7 to 20 business days. Delivery times include only business days, so holidays or weekends may cause slight delays.":
    "Les délais de livraison dépendent du mode d'expédition choisi au moment du paiement. L'expédition standard prend généralement 10 à 30 jours ouvrés, tandis que l'expédition express arrive généralement en 7 à 20 jours ouvrés. Les délais ne comprennent que les jours ouvrés ; les congés ou week-ends peuvent donc entraîner de légers retards.",
  "Will I have to pay duties or extra fees?":
    "Devrai-je payer des taxes ou des frais supplémentaires ?",
  "The prices listed on our website do not include any additional taxes, customs duties, or import fees. These charges may apply depending on your country's regulations and are the responsibility of the recipient. We recommend checking with your local customs office for more information.":
    "Les prix affichés sur notre site ne comprennent pas les taxes supplémentaires, droits de douane ou frais d'importation. Ces frais peuvent s'appliquer selon les règles de votre pays et sont à la charge du destinataire. Nous vous recommandons de vous renseigner auprès de votre bureau de douane local.",
  "What if I'm not happy with my order?":
    "Et si ma commande ne me convient pas ?",
  "After payment, you'll review and approve your book. If you're not happy with it, you can request changes, and our dedicated support team will be happy to assist you.":
    "Après le paiement, vous réviserez et validerez votre livre. S'il ne vous convient pas, vous pouvez demander des modifications et notre équipe d'assistance dédiée se fera un plaisir de vous aider.",
  "How can I reach customer support?":
    "Comment contacter le support client ?",
  "You can contact our customer support team anytime through our support page or by emailing support@monpetithero.shop.":
    "Vous pouvez contacter notre équipe de support client à tout moment via notre page d'assistance ou par e-mail à support@monpetithero.shop.",
  "What languages are your books available in?":
    "Dans quelles langues vos livres sont-ils disponibles ?",
  "Our books are currently available in English, Spanish, Portuguese (Brazil), Arabic, French, Turkish, German, Italian, Dutch and Albanian. We are actively working on adding more languages soon.":
    "Nos livres sont actuellement disponibles en anglais, espagnol, portugais (Brésil), arabe, français, turc, allemand, italien, néerlandais et albanais. Nous travaillons activement à l'ajout de nouvelles langues.",
  "Need help?": "Besoin d'aide ?",
  "Everything you need to know about personalising, ordering and receiving your Mon Petit Hero books.":
    "Tout ce que vous devez savoir sur la personnalisation, la commande et la réception de vos livres Mon Petit Hero.",
  "Still have a question?": "Vous avez encore une question ?",
  "Our support team is happy to help — reach out any time.":
    "Notre équipe d'assistance est ravie de vous aider — contactez-nous à tout moment.",

  // ── Footer ─────────────────────────────────────────────────────────
  "About Mon Petit Hero": "À propos de Mon Petit Hero",
  "Contact us": "Contactez-nous",
  FAQ: "FAQ",
  FAQs: "FAQ",
  Blog: "Blog",
  "Customer Area": "Espace client",
  "My Account": "Mon compte",
  Orders: "Commandes",
  Terms: "Conditions",
  "Privacy Policy": "Politique de confidentialité",
  "Create hyper-personalised storybooks that make your child the hero, with quick customisation and speedy delivery.":
    "Créez des livres d'histoires hyper-personnalisés qui font de votre enfant le héros, avec une personnalisation rapide et une livraison express.",
  "Facebook": "Facebook",
  "Instagram": "Instagram",
  "TikTok": "TikTok",
  "Subscribe to our newsletter": "Abonnez-vous à notre newsletter",
  "Don't miss out on the newest books.":
    "Ne manquez pas les derniers livres.",
  "Email address": "Adresse e-mail",
  Subscribe: "S'abonner",
  "Subscribed. Check your inbox for a confirmation.":
    "Vous êtes abonné. Vérifiez votre boîte de réception pour la confirmation.",
  "All rights reserved": "Tous droits réservés",

  // ── Cards / filler ─────────────────────────────────────────────────
  "Video placeholder": "Aperçu vidéo",
  "Your browser does not support the video tag.":
    "Votre navigateur ne prend pas en charge la balise vidéo.",

  // ── Books listing ──────────────────────────────────────────────────
  "Personalised Books": "Livres personnalisés",
  "Browse personalised storybooks that make your child the hero. Choose from adventures, princess tales, space journeys and more.":
    "Parcourez des livres d'histoires personnalisés qui font de votre enfant le héros. Choisissez parmi les aventures, les contes de princesses, les voyages dans l'espace et plus encore.",
  "Mon Petit Hero collection": "Collection Mon Petit Hero",
  "Personalised storybooks": "Livres d'histoires personnalisés",
  "Your child is the hero of every page. Pick a story, add their photo and name, and receive a beautifully printed book they'll treasure forever.":
    "Votre enfant est le héros de chaque page. Choisissez une histoire, ajoutez sa photo et son nom, et recevez un magnifique livre imprimé qu'il chérira pour toujours.",
  All: "Tous",
  "Showing books for": "Livres affichés pour",
  "Can't decide? Start with our bestsellers":
    "Vous hésitez ? Commencez par nos best-sellers",
  "Every book is fully personalised with your child's photo, name and age — and each printed book order unlocks an extra free story.":
    "Chaque livre est entièrement personnalisé avec la photo, le nom et l'âge de votre enfant — et chaque commande de livre imprimé débloque une histoire gratuite supplémentaire.",
  "Create my child's story": "Créer l'histoire de mon enfant",

  // ── Book detail ────────────────────────────────────────────────────
  "Book not found": "Livre introuvable",
  "Personalise this storybook so your child becomes the hero.":
    "Personnalisez cette histoire pour que votre enfant devienne le héros.",
  "Back to all books": "Retour à tous les livres",
  "Personalised book": "Livre personnalisé",
  "This is the story where": "C'est l'histoire où",
  "becomes the adventure your child is at the center of. Upload a photo of your child, add their first name and age, and watch as they become the hero of this beautifully illustrated, printed storybook.":
    "que votre enfant vit comme son aventure. Téléchargez une photo de votre enfant, ajoutez son prénom et son âge, puis regardez-le devenir le héros de ce livre illustré et imprimé avec soin.",
  "Personalise this book": "Personnaliser ce livre",
  "Talk to support": "Contacter l'assistance",
  "Your child is the hero": "Votre enfant est le héros",
  "Their photo, name and age are woven into every scene.":
    "Sa photo, son nom et son âge sont intégrés à chaque scène.",
  "Personalised to perfection": "Personnalisation parfaite",
  "Pick names, characters and details to make it truly theirs.":
    "Choisissez les prénoms, les personnages et les détails pour le rendre vraiment unique.",
  "Printed with care, shipped fast":
    "Imprimé avec soin, expédié rapidement",
  "Premium softcover books delivered right to your door.":
    "Des livres brochés haut de gamme livrés directement chez vous.",
  "More stories to love": "Plus d'histoires à adorer",

  // ── Stickers ───────────────────────────────────────────────────────
  "Personalised Stickers": "Autocollants personnalisés",
  "Personalised sticker packs for your little girl or boy — printed with your child's favourite characters and delivered with love.":
    "Des packs d'autocollants personnalisés pour votre fille ou votre garçon — imprimés avec les personnages préférés de votre enfant et livrés avec amour.",
  "Sticker fun": "Amusement stickers",
  "Personalised sticker packs": "Packs d'autocollants personnalisés",
  "Colourful, durable stickers starring your child's world — the perfect extra sprinkle on any Mon Petit Hero order.":
    "Des autocollants colorés et durables mettant en scène l'univers de votre enfant — la touche parfaite pour toute commande Mon Petit Hero.",
  "Pick a sticker pack": "Choisissez un pack d'autocollants",
  "Choose the pack that matches your child's favourite characters.":
    "Choisissez le pack qui correspond aux personnages préférés de votre enfant.",
  "Personalise it": "Personnalisez-le",
  "Add your child's name or photo so every sticker feels like theirs.":
    "Ajoutez le nom ou la photo de votre enfant pour que chaque autocollant lui ressemble.",
  "Stick, share, repeat": "Collez, partagez, recommencez",
  "Waterproof and durable — perfect for books, bottles and lunchboxes.":
    "Imperméables et durables — parfaits pour les livres, les gourdes et les boîtes à lunch.",
  "Pair your stickers with a story":
    "Associez vos autocollants à une histoire",
  "Add a fully personalised storybook to your order and your child gets a hero, an adventure and stickers to match.":
    "Ajoutez un livre d'histoires entièrement personnalisé à votre commande et votre enfant recevra un héros, une aventure et des autocollants assortis.",
  "Browse books": "Parcourir les livres",

  // ── Support ────────────────────────────────────────────────────────
  "We're here to help": "Nous sommes là pour vous aider",
  "How can we help you?": "Comment pouvons-nous vous aider ?",
  "Find answers to common questions, get in touch with the team, or catch up on the details that matter.":
    "Trouvez des réponses aux questions courantes, contactez l'équipe ou informez-vous sur les détails importants.",
  "Quick answers on ordering, personalisation, shipping, refunds and more.":
    "Des réponses rapides sur les commandes, la personnalisation, l'expédition, les remboursements et plus encore.",
  "Read FAQs": "Lire les FAQ",
  "Send the team a message and hear back within 24 hours.":
    "Envoyez un message à l'équipe et recevez une réponse sous 24 heures.",
  "Contact the team": "Contacter l'équipe",
  "Terms & Conditions": "Conditions générales",
  "The fine print on orders, prints, payments and our promise to you.":
    "Les détails sur les commandes, les impressions, les paiements et nos engagements.",
  "Read terms": "Lire les conditions",
  "How we handle the photos, names and details you share with us.":
    "Comment nous traitons les photos, les noms et les informations que vous partagez avec nous.",
  "Read privacy policy": "Lire la politique de confidentialité",
  "Need a human? Email us at": "Besoin d'un humain ? Écrivez-nous à",
  "Send a message": "Envoyer un message",

  // ── Terms & Conditions ─────────────────────────────────────────────
  "Back to support": "Retour à l'assistance",
  "Last updated: January 2026": "Dernière mise à jour : janvier 2026",
  "1. The service": "1. Le service",
  "monpetithero.shop sells personalised children's storybooks. Every book is created individually from details you provide, including your child's photo, name and age.":
    "monpetithero.shop vend des livres d'histoires personnalisés pour enfants. Chaque livre est créé individuellement à partir des informations que vous fournissez, notamment la photo, le nom et l'âge de votre enfant.",
  "2. Orders & payment": "2. Commandes et paiement",
  "All prices are shown in the currency you select. Payment is taken at checkout and your book is printed after you approve the preview. Each printed book order unlocks one extra free story generation on your account.":
    "Tous les prix s'affichent dans la devise que vous choisissez. Le paiement est effectué lors de la commande et votre livre est imprimé après votre validation de l'aperçu. Chaque commande de livre imprimé débloque une génération d'histoire gratuite supplémentaire sur votre compte.",
  "3. Refunds": "3. Remboursements",
  "You can receive a full refund if your book has not been printed yet, or a partial refund if it has been printed but not yet shipped. Once printed and shipped, we are unable to offer a refund. Contact us at support@monpetithero.shop to request one.":
    "Vous pouvez obtenir un remboursement intégral si votre livre n'a pas encore été imprimé, ou partiel s'il a été imprimé mais pas encore expédié. Une fois imprimé et expédié, nous ne pouvons pas offrir de remboursement. Contactez-nous à support@monpetithero.shop pour en faire la demande.",
  "4. Shipping & duties": "4. Expédition et taxes",
  "Standard shipping usually takes 10–30 business days; express takes 7–20 business days. Prices listed do not include taxes, customs duties or import fees, which are the recipient's responsibility.":
    "L'expédition standard prend généralement 10 à 30 jours ouvrés ; l'express 7 à 20 jours ouvrés. Les prix affichés ne comprennent pas les taxes, droits de douane ou frais d'importation, qui relèvent du destinataire.",
  "5. Your content": "5. Votre contenu",
  "You retain all rights to the photos and details you provide. We only use them to create and print your order and never share them with third parties.":
    "Vous conservez tous les droits sur les photos et informations que vous fournissez. Nous ne les utilisons que pour créer et imprimer votre commande et ne les partageons jamais avec des tiers.",
  "6. Contact": "6. Contact",
  "Questions about these terms? Reach us at support@monpetithero.shop.":
    "Des questions sur ces conditions ? Contactez-nous à support@monpetithero.shop.",
  "The terms and conditions that apply to ordering personalised storybooks from Mon Petit Hero.":
    "Les conditions générales applicables à la commande de livres d'histoires personnalisés chez Mon Petit Hero.",

  // ── Privacy Policy ─────────────────────────────────────────────────
  "1. What we collect": "1. Ce que nous collectons",
  "To create your personalised books we collect your contact details (name, email, shipping address) and the personalised content you provide, such as your child's photo, name and age.":
    "Pour créer vos livres personnalisés, nous collectons vos coordonnées (nom, e-mail, adresse de livraison) et le contenu personnalisé que vous fournissez, comme la photo, le nom et l'âge de votre enfant.",
  "2. How we use it": "2. Comment nous l'utilisons",
  "We use this information to build and print your order, keep you updated on its progress, and improve our service. We never sell your personal data to anyone.":
    "Nous utilisons ces informations pour créer et imprimer votre commande, vous tenir informé de son avancement et améliorer notre service. Nous ne vendons jamais vos données personnelles à quiconque.",
  "3. Photos & children's information": "3. Photos et informations des enfants",
  "The photos and details you upload are used only to generate your book. They are processed securely and are never published, shared or used for any other purpose without your consent.":
    "Les photos et informations que vous téléchargez servent uniquement à générer votre livre. Elles sont traitées de façon sécurisée et ne sont jamais publiées, partagées ou utilisées à d'autres fins sans votre consentement.",
  "4. Payments": "4. Paiements",
  "Card payments are processed by trusted third-party payment providers. We do not store your full card details on our servers.":
    "Les paiements par carte sont traités par des prestataires de paiement tiers de confiance. Nous ne stockons pas l'intégralité des données de votre carte sur nos serveurs.",
  "5. Your rights": "5. Vos droits",
  "You may request a copy, correction or deletion of your personal data at any time by emailing support@monpetithero.shop.":
    "Vous pouvez demander une copie, une correction ou la suppression de vos données personnelles à tout moment en envoyant un e-mail à support@monpetithero.shop.",
  "How Mon Petit Hero collects, uses and protects the personal details and photos you share with us.":
    "Comment Mon Petit Hero collecte, utilise et protège les informations personnelles et les photos que vous partagez avec nous.",

  // ── Contact ────────────────────────────────────────────────────────
  "We'd love to hear from you": "Nous serions ravis de vous lire",
  "Questions about an order, a personalisation, or the perfect book for your child? Send us a message.":
    "Une question sur une commande, une personnalisation ou le livre parfait pour votre enfant ? Envoyez-nous un message.",
  "Email us": "Écrivez-nous",
  "Chat with us": "Discutez avec nous",
  "On Facebook, Instagram and TikTok":
    "Sur Facebook, Instagram et TikTok",
  "Response time": "Délai de réponse",
  "We usually reply within 24 hours.":
    "Nous répondons généralement sous 24 heures.",
  "Message sent!": "Message envoyé !",
  "Thanks for reaching out. Our team will get back to you within 24 hours.":
    "Merci de nous avoir contactés. Notre équipe vous répondra sous 24 heures.",
  Name: "Nom",
  "Your name": "Votre nom",
  Email: "E-mail",
  Subject: "Objet",
  "What is this about?": "De quoi s'agit-il ?",
  Message: "Message",
  "Tell us how we can help…": "Dites-nous comment nous pouvons vous aider…",
  "Send message": "Envoyer le message",

  // ── Blog ───────────────────────────────────────────────────────────
  "The Mon Petit Hero blog": "Le blog Mon Petit Hero",
  "Stories behind the stories": "Les histoires derrière les histoires",
  "Ideas, tips and behind-the-scenes peeks to help you raise little readers and big dreamers.":
    "Des idées, des conseils et des aperçus des coulisses pour élever de petits lecteurs et de grands rêveurs.",
  "Bedtime tips": "Conseils pour le coucher",
  "Why personalised books make bedtime easier":
    "Pourquoi les livres personnalisés facilitent le coucher",
  "When your child is the hero, they beg for one more page. Here's the science behind personalised bedtime stories — and how to make them a habit.":
    "Quand votre enfant est le héros, il réclame une page de plus. Voici la science derrière les histoires du soir personnalisées — et comment en faire une habitude.",
  "Coming soon": "Bientôt disponible",
  Ideas: "Idées",
  "10 adventures to spark your child's imagination":
    "10 aventures pour stimuler l'imagination de votre enfant",
  "From dragon rescues to cosmic journeys, these story ideas are a great starting point for your child's very own book.":
    "Des sauvetages de dragons aux voyages cosmiques, ces idées d'histoires sont un excellent point de départ pour le livre de votre enfant.",
  News: "Actualités",
  "Personalisation: how your child's book is made":
    "Personnalisation : comment le livre de votre enfant est fabriqué",
  "A peek behind the scenes at how we turn a photo, a name and a story idea into a beautifully printed keepsake.":
    "Un aperçu des coulisses : comment nous transformons une photo, un prénom et une idée d'histoire en un souvenir magnifiquement imprimé.",
  "Read more": "Lire la suite",
  "Meanwhile, why not start your child's own story?":
    "En attendant, pourquoi ne pas commencer l'histoire de votre enfant ?",
  "Create a personalised book": "Créer un livre personnalisé",

  // ── Not found / errors ─────────────────────────────────────────────
  "This page turned into a blank book":
    "Cette page s'est transformée en livre vierge",
  "We couldn't find what you were looking for.":
    "Nous n'avons pas trouvé ce que vous cherchiez.",
  "Back home": "Retour à l'accueil",
  "Something went wrong": "Une erreur est survenue",
  "We hit a snag while loading this page. You can retry, or head back home.":
    "Un problème est survenu pendant le chargement de cette page. Vous pouvez réessayer ou revenir à l'accueil.",
  "Try again": "Réessayer",
  "Go home": "Aller à l'accueil",
  "An unexpected error occurred.": "Une erreur inattendue s'est produite.",
  Reload: "Recharger",

  // ── Auth pages ─────────────────────────────────────────────────────
  "Welcome back": "Bon retour",
  "Sign in to continue your stories.":
    "Connectez-vous pour continuer vos histoires.",
  "Create your account": "Créez votre compte",
  "Every new account starts with free story generations.":
    "Chaque nouveau compte commence avec des générations d'histoires gratuites.",
  "Redirecting to your dashboard...":
    "Redirection vers votre tableau de bord...",

  // ── Welcome ────────────────────────────────────────────────────────
  Welcome: "Bienvenue",
  "Let's create your first story hero":
    "Créons le héros de votre première histoire",
  "Follow these quick steps. You'll be reading magical bedtime stories in no time.":
    "Suivez ces étapes rapides. Vous lirez bientôt de merveilleuses histoires du soir.",
  "Add your child's photo": "Ajoutez la photo de votre enfant",
  "One clear, smiling photo keeps the hero recognizable on every page.":
    "Une photo claire et souriante permet de reconnaître le héros à chaque page.",
  "Generate the story": "Générez l'histoire",
  "Pick a theme and we write and illustrate it in minutes.":
    "Choisissez un thème et nous écrivons et illustrons l'histoire en quelques minutes.",
  "Read it or order a printed book":
    "Lisez-la ou commandez un livre imprimé",
  "Every printed book you order unlocks 1 extra free story.":
    "Chaque livre imprimé commandé débloque 1 histoire gratuite supplémentaire.",
  "Create My First Story": "Créer ma première histoire",
  "Browse Templates": "Parcourir les modèles",

  // ── Upload / camera / pack / shared ────────────────────────────────
  "Uploading photos...": "Téléchargement des photos...",
  "Processing...": "Traitement...",
  "Drag & drop photos here": "Glissez-déposez vos photos ici",
  "or click to browse": "ou cliquez pour parcourir",
  "Select Photos": "Sélectionner des photos",
  "Supports JPG, PNG, WEBP • Max 50 files":
    "Formats acceptés : JPG, PNG, WEBP • Max 50 fichiers",
  "Your Gallery": "Votre galerie",
  "No images yet. Start by generating some!":
    "Pas encore d'images. Commencez à en générer !",
  "Image Preview": "Aperçu de l'image",
  "Download Image": "Télécharger l'image",
  "No image yet": "Pas encore d'image",
  "Generated image": "Image générée",
  "Loading image": "Chargement de l'image",
  "Image coming soon": "Image bientôt disponible",
  "Loading…": "Chargement…",
  "Opening your book…": "Ouverture de votre livre…",
  "Previous slide": "Diapositive précédente",
  "Next slide": "Diapositive suivante",
  "Starting pack generation...": "Démarrage de la génération du pack...",
  "Pack generation started successfully!":
    "Génération du pack lancée avec succès !",
  "Failed to start generation": "Échec du démarrage de la génération",

  // ── Dynamic fragments (From $) ─────────────────────────────────────
  "From ": "À partir de ",
};