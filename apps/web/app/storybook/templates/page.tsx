...
export default async function StorybookTemplatesPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className="text-sm uppercase tracking-[0.3em] text-primary font-medium">Educational & Moral Templates</p>
        <h1 className="font-display text-4xl font-bold text-violet-deep">Pick a story starter with value</h1>
      ...
      {templates.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-border bg-white p-10 text-center text-muted-foreground">
          ...
          <Link href="/storybook/create" className="text-primary underline underline-offset-4">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {templates.map((template) => (
            <div key={template.id} className="flex flex-col justify-between rounded-3xl border border-border bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
              <div>
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <span className="inline-block rounded-full bg-buttercup/10 px-3 py-1 text-xs font-semibold uppercase tracking-wider text-primary">
                      {template.category}
                    </span>
                    <h3 className="mt-2 text-2xl font-semibold text-violet-deep">{template.name}</h3>
                  </div>
                  <BookMarked className="h-6 w-6 text-primary flex-shrink-0" />
                </div>

                <p className="mt-3 text-sm text-muted-foreground">{template.description ?? "Bring this template to life with your child’s photos."}</p>
              ...
              <div className="mt-6 flex items-center justify-between pt-4 border-t border-border">
                ...
                <Link
                  ...
                  className="inline-flex items-center justify-center rounded-full bg-primary px-5 py-2 text-sm font-semibold text-white hover:bg-violet-deep transition-colors"
                >
                  Use template
                </Link>
              </div>
